...
void blk_irq_handler(struct interrupt_desc *desc)
{
    blk *blk = (virtio::blk *)desc->obj;
    if (blk->ack_irq()) {
...
#ifdef AARCH64_PORT_STUB
    u8 bus, device, func; /* BEWARE, written by get_bdf using references */
    pci_dev.get_bdf(bus, device, func);
    pci::register_pci_irq(bus, device, func, this, &blk_irq_handler);
