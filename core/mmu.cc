...
class linear_page_mapper :
    ...
    mattr_t mattr;
public:
    linear_page_mapper(phys start, size_t size, mattr_t mattr = mattr_default) :
        start(start), end(start + size), mattr(mattr) {}
    template<int N>
    bool page(hw_ptep<N> ptep, uintptr_t offset) {
        phys addr = start + offset;
        assert(addr < end);
        ptep.write(make_leaf_pte(ptep, addr, mmu::perm_rwx, mattr));
        return true;
    }
};
                ...
                size_t slop, mattr_t mattr)
{
    ...
    linear_page_mapper phys_map(addr, size, mattr);
