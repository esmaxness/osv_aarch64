...
static struct spinlock gic_spinlock;
class gic_driver *gic_driver;
...
void gic_driver::init_cpu(int smp_idx)
{
    ...
    */
    ...
    debug_early("reading my cpu target mask...\n");
    ...
    debug_early("...read cpu mask yeah!\n");

    debug_early("disabling ppi interrupts...\n");
    /* disable all PPI interrupts */
    this->gicd.write_reg_raw((u32)gicd_reg_irq1::GICD_ICENABLER, 0, 0xffff0000);
    debug_early(" enabling sgi interrupts...\n");
    /* enable all SGI interrupts */
    this->gicd.write_reg_raw((u32)gicd_reg_irq1::GICD_ISENABLER, 0, 0x0000ffff);
    /* set priority on SGI and PPI (at least bits [7:4] must be implemented) */
    debug_early("setting priority sgi,ppi...\n");

    for (int i = 0; i < 32; i += 4) {
        this->gicd.write_reg_raw((u32)gicd_reg_irq8::GICD_IPRIORITYR, i, 0xc0c0c0c0);
    }

    debug_early("setting priority mask register for CPU...\n");

    for (int i = 0; i < 32; i += 4) {
        this->gicc.write_reg(gicc_reg::GICC_PMR, 0xf0);
    }

    debug_early("enabling cpu interface...\n");
...
void gic_driver::init_dist(int smp_idx)
{
    ...
    debug_early("disabling first the gic distributor...\n");
    ...
    this->nr_irqs = ((this->gicd.read_reg(gicd_reg::GICD_TYPER) & 0x1f) + 1) * 32;
    debug_early_u64("number of supported IRQs: ", (u64)this->nr_irqs);

    debug_early("setting all SPIs to level-sensitive...\n");
    ...
    debug_early("target all SPIs to this cpu only, and set prio...\n");
    /* send all SPIs to this only, set priority */
    for (int i = 32; i < this->nr_irqs; i += 4) {
        this->gicd.write_reg_raw((u32)gicd_reg_irq8::GICD_ITARGETSR, i, mask);
        this->gicd.write_reg_raw((u32)gicd_reg_irq8::GICD_IPRIORITYR, i, 0xc0c0c0c0);
    }

    debug_early("mask all SPIs...\n");
    /* disable all SPIs */
    for (int i = 32; i < this->nr_irqs; i += 32)
        this->gicd.write_reg_raw((u32)gicd_reg_irq1::GICD_ICENABLER, i / 8, 0xffffffff);

    /* enable distributor interface */
    debug_early("enable distributor interface...\n");
    gicd_ctlr |= 1;
    this->gicd.write_reg(gicd_reg::GICD_CTLR, gicd_ctlr);
